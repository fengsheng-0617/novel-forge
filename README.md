# 织文 NovelForge —— AI 小说创作工坊

大型、可完全自定义的本地 AI 小说创作工具：**灵感点子 → 世界观设定 → 人物群像 → 卷章大纲 → 章节正文** 全流水线，每一步 AI 产出都可**预览后入库、自由编辑、重新生成、单点精修、整体回退（撤销）**；支持接入 OpenAI 兼容接口（含国产各家）、Gemini、Grok/xAI；内置**离线模拟引擎**与示例小说，**无需任何 API Key 即可完整体验全部功能**。
...
| ④ 卷章大纲 | 分卷、逐章 目标+节拍+登场+视角+字数 | AI 全书大纲（卷弧线）、追加 N 章、单章精修、手动加章/排序/改卷/重排 |
...
| 导出 | 一键下载 | `book.md` 成书 / `manuscript.md` 全创作底稿 / `book.txt` 纯文本 / `backup.json` 项目备份 |
...
设置 → **提示词模板库**：18 套模板按阶段分组，可在线编辑 System/User 提示词（占位符 `{{ideaText}}`、`{{bibleText}}`、`{{charsText}}`、`{{curRowText}}`、`{{contText}}` 等由系统自动注入），改动即刻生效于后续所有生成；可一键「全部重置为内置默认」。JSON 类动作的输出规范（纯 JSON、防代码块）与正文类动作的排版规范（纯正文、禁说明语）由系统自动追加。
...
│  ├─ templates.js                # 18 套内置提示词模板
│  ├─ context.js                  # 项目上下文组装与预算裁剪
│  ├─ actions.js                  # 18 个生成动作（组装/解析/应用）
│  ├─ pipeline.js                 # 无人值守流水线
...
本仓库是**小说写作引擎的唯一真源**，定位专一：只做 点子→设定→人物→大纲→正文 的创作工具。
...
node scripts/test-gen.js          # 生成引擎全链路          （35 项）
...
后端三套回归可在不配置任何 Key 的情况下运行（使用内置模拟引擎）。
