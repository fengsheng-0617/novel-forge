...
A large, fully customizable local AI creation tool that unifies **novel writing** with **content imitation / continuation / rewriting, UN Security Council resolution imitation, and academic cold-emails** under one capability framework, and supports **multilingual writing**. Novel pipeline: **spark of an idea → world-setting/bible → characters → volume & chapter outline → chapter prose**. At every step the AI output can be **previewed, then committed to the store, freely edited, regenerated, spot-refined, or wholly rolled back (undo)**; supports OpenAI-compatible endpoints (including the domestic Chinese providers), Gemini, Grok/xAI; ships with a built-in **offline mock engine** and sample novel, so **no API Key is required to fully experience all features**.
...
1. Writes default settings (presets for 12 providers + 18 prompt templates);
...
## Creation Workflow (six stages + export)
...
| ④ Volume & chapter outline | Per-volume, per-chapter goal + beats + cast + POV + word count | AI full-book outline (volume arcs), append N chapters, refine one chapter, manual add-chapter / reorder / change-volume / rearrange |
...
| Export | One-click download | `book.md` finished book / `manuscript.md` full creation draft / `book.txt` plain text / `backup.json` project backup |
...
│  ├─ templates.js                # 18 built-in prompt templates
│  ├─ context.js                  # project context assembly & budget trimming
│  ├─ actions.js                  # 18 generation actions (assembly / parse / apply)
...
This repository is the **single source of truth for the novel-writing engine**, with a focused scope: only the idea → world-setting → characters → outline → prose creation tool.
...
node scripts/test-gen.js          # full generation-engine pipeline  (35 items)
...
The three server-side regression suites can run without configuring any Key (using the built-in mock engine).
